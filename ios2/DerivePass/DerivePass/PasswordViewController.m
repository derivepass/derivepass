...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    if ([[segue identifier] isEqualToString:@"ToApplications"]) {
        ApplicationsTableViewController* c =
...
- (IBAction)onPasswordChange:(id)sender {
    ...
    static char** alphabet[] = {
        smile, gesture, animal, food, object };
    static unsigned int alphabet_size[] = {
        ARRAY_SIZE(smile), ARRAY_SIZE(gesture),
    ...
    self.emojiLabel.text =
        [NSString stringWithUTF8String: emoji_fingerprint];
}


- (IBAction)onSubmitPassword:(id)sender {
    [self performSegueWithIdentifier: @"ToApplications"
                              sender: self];
}
